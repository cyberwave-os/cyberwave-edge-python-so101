...
# Position reads (addr 56) are critical - retry until success to avoid losing telemetry
_POSITION_READ_MAX_RETRIES = 3
_POSITION_READ_RETRY_DELAY_SEC = 0.008  # 8ms between retries
...
class FeetechMotorsBus(MotorsBus):
    ...
    def __init__(
        ...
        self._port_lock = threading.RLock()  # Serialize port access (status thread + main loop)
    ...
    def _preflight_check_motors(self, max_retries: int = 3) -> bool:
        ...
        try:
            ...
            # Check that all motors report the same model number (consistency check)
            if found_models:
                unique_models = set(found_models.values())
                if len(unique_models) > 1:
                    logger.error(
                        "Model number mismatch across motors: %s",
                        {motor_id: model for motor_id, model in found_models.items()},
                    )
                    return False
                logger.info("All motors report consistent model number: %s", next(iter(unique_models)))
            ...
            logger.info(
                "Pre-flight check passed: all motors responded with consistent firmware%s",
                f" (v{next(iter(firmware_versions.values()))})" if firmware_versions else "",
            )
            return True
    ...
    def sync_read_positions(
        ...
        Position reads (addr 56) are critical - retries aggressively on "Port is in use".
    ...
    def sync_read(
        ...
        Args:
            ...
            num_retry: Number of retry attempts on communication failure
            use_sequential: If True, use sequential reads instead of GroupSyncRead.
        ...
        raw_values = self.sync_read_register(
            data_name, motor_ids, num_retry=num_retry, decode=True, use_sequential=use_sequential
        )
